...
| `mode` | Recommended | The chat mode: `ask`, `edit`, or `agent`. Defaults to current mode |
...
- If `tools` are specified and the current mode is `ask` or `edit`, the default mode becomes `agent`
